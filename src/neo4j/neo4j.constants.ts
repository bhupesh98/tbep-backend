  ...
  switch (order) {
    case 0:
      return `MATCH (g1:Gene) WHERE g1.ID IN $geneIDs
        ...
        WITH [conn IN COLLECT({gene1: g1.ID, gene2: g2.ID, score: r.score}) WHERE conn.gene2 IS NOT NULL] AS connections, apoc.coll.toSet(COLLECT(g1 { .ID, .Gene_name, .Description})) AS genes
